...
EGUI_API Vec2 EInput::GetWindowPos() {
	RECT rect = { NULL };
...
EGUI_API Vec2 EInput::GetWindowSize() {
	RECT rect = { NULL };
	if (GetWindowRect(EGui.hwnd, &rect)) // Idk why, please find a soloution for this but it requires extra amounts? 16 and 39
		return Vec2(rect.right - rect.left - (EGui.Windowed ? 16 : 0), rect.bottom - rect.top - (EGui.Windowed ? 39 : 0));
...
EGUI_API bool EInput::IsMouseHoveringRect(Vec2 pos, Vec2 size)
{
...
EGUI_API bool EInput::ButtonBehaviour(Vec2 pos, Vec2 size, int button_type) {
	if (!window.IsWindowParent())
		return false;
...
EGUI_API bool EInput::IsKeyDown(int key) {
	if (!window.IsWindowParent())
		return false;
...
EGUI_API bool EInput::IsKeyPressed(int key)
{
	if (!window.IsWindowParent())
		return false;
...
EGUI_API Vec2 EInput::GetMousePos() {
	POINT p;
	GetCursorPos(&p);
