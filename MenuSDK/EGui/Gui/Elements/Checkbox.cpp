...
bool EGuiMain::Checkbox(const char* title, bool* state) {
	...
	if (*state)
		checkbox_alpha[GetItemIdentifier()] = Math.Clamp(Animations.lerp(checkbox_alpha[GetItemIdentifier()], 255.f, delta_time * 8), 0.f, 255.f);
	else
		checkbox_alpha[GetItemIdentifier()] = Math.Clamp(Animations.lerp(checkbox_alpha[GetItemIdentifier()], 0.f, delta_time * 8), 0.f, 255.f);
