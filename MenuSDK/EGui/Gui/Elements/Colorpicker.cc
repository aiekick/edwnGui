...
EGUI_API bool EGuiMain::ColorPicker(const char* title, Color* selected) {
    ...
    drawList.FilledRectangle(NextDrawPos + Vec2(Size.x - 10, 2), Vec2(8, 14), *selected);
    ...
    if (this_state[GetItemIdentifier()]) {
        ...
        drawList.FilledRectangle(NextDrawPos + Vec2(0, Size.y), Size + Vec2(0, Size.y * 2), EGuiColors.ElementBackColor);
        drawList.Rectangle(NextDrawPos, Size + Vec2(0, Size.y * 2), EGuiColors.MenuTheme);
        ...
        for (int i = 0; i < 20; ++i) {
            for (int j = 0; j < 20; ++j) {
                ...
                drawList.FilledRectangle(NextDrawPos + Vec2(i * 10, Size.y + j * 10), Vec2(10, 10), color);
